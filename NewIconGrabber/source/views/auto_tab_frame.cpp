...
AutoTabFrame::AutoTabFrame() {
    ...
    this->registerAction("Previous Tab", brls::ControllerButton::BUTTON_LB, [&](brls::View* view) {
        if (this->isOnTop) {
            this->focus2LastTab();
        }
        return true;
    }, true);
    this->registerAction("Next Tab", brls::ControllerButton::BUTTON_RB, [&](brls::View* view) {
        if (this->isOnTop) {
            this->focus2NextTab();
        }
        return true;
...
void AutoTabFrame::setTabAttachedView(brls::View* newContent) {
    brls::Logger::debug("setTabAttachedView");
...
void AttachedView::setTabActionAvailable(brls::ControllerButton button, bool available) {
    this->setActionAvailable(button, available);
    if (this->tab) this->tab->setActionAvailable(button, available);
}
